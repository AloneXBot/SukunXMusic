...
from SukunXMusic import YouTube, app
from SukunXMusic.core.call import Sukun
from SukunXMusic.misc import SUDOERS, db
from SukunXMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from SukunXMusic.utils.decorators.language import languageCB
from SukunXMusic.utils.formatters import seconds_to_min
from SukunXMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from SukunXMusic.utils.stream.autoclear import auto_clean
from SukunXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Sukun.pause_stream(chat_id)
        ...
        await Sukun.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Sukun.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Sukun.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Sukun.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Sukun.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Sukun.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
